        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white dark:bg-gray-800 overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6 text-black">
                    <form action="{{ route('share.pinjam.update', $pinjaman->id) }}" method="POST" enctype="multipart/form-data">
                        @csrf
                        @method('PUT')
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                            <div>
                                ...
                                <select name="anggota_id" id="anggota_id" class="w-full px-3 py-2 border rounded-lg text-black">
                                    ...
                                    @foreach($anggota as $a)
                                        <option value="{{ $a->id }}" {{ $pinjaman->anggota_id == $a->id ? 'selected' : '' }}>
                            ...
                            <div>
                                ...
                                <input type="number" name="jumlah" id="jumlah" class="w-full border-gray-300 rounded p-2 text-black" value="{{ $pinjaman->jumlah }}" required>
                            </div>

                            <div>
                                <label for="bunga" class="block text-sm font-medium text-white">Bunga (%)</label>
                                <input type="number" name="bunga" id="bunga" class="w-full border-gray-300 rounded p-2 text-black" step="0.01" value="{{ $pinjaman->bunga }}" required>
                            </div>

                            <div>
                                <label for="tenor" class="block text-sm font-medium text-white">Tenor (bulan)</label>
                                <input type="number" name="tenor" id="tenor" class="w-full border-gray-300 rounded p-2 text-black" value="{{ $pinjaman->tenor }}" required>
                            </div>

                            <div>
                                <label for="tanggal_pinjam" class="block text-sm font-medium text-white">Tanggal Pinjam</label>
                                <input type="date" name="tanggal_pinjam" class="w-full border-gray-300 rounded p-2 text-black" value="{{ $pinjaman->tanggal_pinjam }}" required>
                            </div>

                            <div>
                                <label for="tanggal_jatuh_tempo" class="block text-sm font-medium text-white">Tanggal Jatuh Tempo</label>
                                <input type="date" name="tanggal_jatuh_tempo" class="w-full border-gray-300 rounded p-2 text-black" value="{{ $pinjaman->tanggal_jatuh_tempo }}" required>
                            </div>

                            <div>
                                <label for="tanggal_pengajuan" class="block text-sm font-medium text-white">Tanggal Pengajuan</label>
                                <input type="date" name="tanggal_pengajuan" class="w-full border-gray-300 rounded p-2 text-black" value="{{ $pinjaman->tanggal_pengajuan }}" required>
                            </div>

                            <div>
                                <label for="tanggal_persetujuan" class="block text-sm font-medium text-white">Tanggal Persetujuan</label>
                                <input type="date" name="tanggal_persetujuan" class="w-full border-gray-300 rounded p-2 text-black" value="{{ $pinjaman->tanggal_persetujuan }}" required>
                            </div>
                        </div>
